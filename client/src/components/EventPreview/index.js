  ...
  return (
    <Link to={`/event/${props.id}`} className={styles.link}>
      <div className={styles.item}>
        <span className={styles.name}>{props.name ?? "Event name"}</span>{" "}
        <br></br>
        <div className="d-flex justify-content-between align-items-center ">
          <span className={styles.time}>
            <BIcon icon="calendar" />{" "}
            {`${getDate(props.time)} ${getTime(props.time)}` ?? ""}
          </span>
          <span className={styles.location}>
            <BIcon icon="geo-alt-fill" />{" "}
            {props.address?.split(", ").splice(0, 2).join(", ")}
          </span>
        </div>
        <div className="d-flex justify-content-between align-items-center ">
          ...
          <span className={styles.createdSpan}>
            utworzone {getDate(props.createdTime)} o godzinie{" "}
            {getTime(props.createdTime)}
          </span>
        </div>
      </div>
    </Link>
